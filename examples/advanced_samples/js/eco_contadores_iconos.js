...
submarine.controller.basicDynamic = function(idName, className, options){
    ...
    // Radius scale
    self.rScale = d3.scale.sqrt().domain([0,300]).range([500,1500]);
                    ...
                    'point':{
                        'zoom':function(selection){
                                selection.transition().duration(0)
                                    ...
                                    .attr("x",function(d) { return self.mapChart.project(d.geometry.coordinates)[0] - d.radius/2})
                                    .attr("y",function(d) { return self.mapChart.project(d.geometry.coordinates)[1] - d.radius/2});
                        },

                        'enter': function(selection){
                                selection
                                    .attr("xlink:href", "img/bike_icon_3d.png")
                                    .attr("width",function(d) {
                                        var datum = self.getDatumFromId(d.id);
                                        var radius = self.rScale(datum);
                                        d.radius = self.mapChart.getRealPixels(radius, d.geometry.coordinates[1]);
    ...
    $(document).ready(function()
    {
        ...
        self.changeSliderCallBack = function (value){
            ...
            self.mapChart.updateValues(["bikes"]);
        };
        ...
        self.mapChart.addLayer("bikes",{'points':'image'});
                ...
                if(mapData!=null){
                    ...
                    d3.json(self.baseJUrl+self.weatherFile, function(weatherData)
                    {
                        if(weatherData!=null){
                            ...
                            self.renderFile(self.geoFile, "bikes");
                            self.fillWeatherInfo();
                        }
                        else
                        {
